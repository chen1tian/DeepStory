from __future__ import annotations

import json
import os
from pathlib import Path
from typing import Any

import aiofiles
import structlog

from app.config import settings

log = structlog.get_logger()


def _presets_dir() -> Path:
    d = settings.data_dir / "presets"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _preset_path(preset_id: str) -> Path:
    return _presets_dir() / f"{preset_id}.json"


async def save_preset(preset_id: str, data: dict[str, Any]) -> None:
    path = _preset_path(preset_id)
    tmp = path.with_suffix(".json.tmp")
    async with aiofiles.open(tmp, "w", encoding="utf-8") as f:
        await f.write(json.dumps(data, ensure_ascii=False, indent=2))
    os.replace(str(tmp), str(path))


async def load_preset(preset_id: str) -> dict[str, Any] | None:
    path = _preset_path(preset_id)
    if not path.exists():
        return None
    async with aiofiles.open(path, "r", encoding="utf-8") as f:
        return json.loads(await f.read())


async def list_presets() -> list[dict[str, Any]]:
    d = _presets_dir()
    result = []
    for f in sorted(d.iterdir()):
        if f.suffix == ".json":
            async with aiofiles.open(f, "r", encoding="utf-8") as fh:
                result.append(json.loads(await fh.read()))
    return result


async def delete_preset_file(preset_id: str) -> bool:
    path = _preset_path(preset_id)
    if path.exists():
        path.unlink()
        return True
    return False


async def clear_default_flag() -> None:
    """Remove is_default from all presets."""
    d = _presets_dir()
    for f in d.iterdir():
        if f.suffix == ".json":
            async with aiofiles.open(f, "r", encoding="utf-8") as fh:
                data = json.loads(await fh.read())
            if data.get("is_default"):
                data["is_default"] = False
                tmp = f.with_suffix(".json.tmp")
                async with aiofiles.open(tmp, "w", encoding="utf-8") as fh:
                    await fh.write(json.dumps(data, ensure_ascii=False, indent=2))
                os.replace(str(tmp), str(f))
